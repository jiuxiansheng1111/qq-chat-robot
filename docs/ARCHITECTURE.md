...
`app/services/music.py` 仅在成员明确使用 `/点歌` 或 `@机器人 点歌` 时查询 Deezer。响应中的试听、封面和落地页 URL 均经过 HTTPS 域名白名单校验，再转换为 OneBot `custom music` 卡片。当前只发送平台提供的约 30 秒试听和歌曲页面，不下载、不缓存、不重新分发完整版权音频；音乐卡片失败时降级为试听与平台链接。
