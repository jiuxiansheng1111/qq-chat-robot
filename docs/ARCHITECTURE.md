...
当前实现额外包含三类受限数据：长期记忆按群和成员隔离，只保存成员明确要求记住的最近 20 条内容；活跃度只按日期累计消息条数；群聊风格只保存消息长度、标点、颜文字和固定安全口头语的匿名聚合计数，不保存普通消息原文。每日夺舍结果按群和日期保存，并记录 `random` 或 `targeted` 模式。随机模式从当天发言达到 15 条的成员中抽取；指向模式通过 OneBot `get_group_member_info` 校验并读取被 @ 成员的群名片。夺舍后的个体风格学习临时读取 OneBot 最近历史，只持久化不含事实和隐私的风格摘要，不保存原始样本。

## 音乐点歌

`app/services/music.py` 仅在成员明确使用 `/点歌` 或 `@机器人 点歌` 时查询网易云。优先发送 OneBot `music` 的 `type=163` 卡片；网易云结果按标题精确匹配、原唱优先，并过滤 Cover、Remix、KTV、伴奏和纯音乐版本。整串搜索不明确时，会按最长歌名后缀重试，例如 `夜鹿 春泥棒` 会回退搜索 `春泥棒`。卡片失败时降级为网易云页面链接，不下载或重新分发完整版权音频。

退出状态单独按群和日期保存。被选中的成员或管理员退出后，当天随机与指向模式都不会再次启用，次日因日期变化自动恢复。
