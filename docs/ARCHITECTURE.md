...
当前实现额外包含三类受限数据：长期记忆按群和成员隔离，只保存成员明确要求记住的最近 20 条内容；活跃度只按日期累计消息条数；群聊风格只保存消息长度、标点、颜文字和固定安全口头语的匿名聚合计数，不保存普通消息原文。每日夺舍结果按群和日期保存，并记录 `random` 或 `targeted` 模式。随机模式从当天发言达到 15 条的成员中抽取；指向模式通过 OneBot `get_group_member_info` 校验并读取被 @ 成员的群名片。
