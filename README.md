...
- `@机器人 点歌 歌名` 使用 Deezer 全球曲库搜索并发送可点击播放的 30 秒试听音乐卡片；歌手名可选，搜索不区分大小写，无需 API Key，也不会影响普通 @ 对话。
...
POST /onebot/webhook
   ...
   ├─ 内置指令与自定义插件
   ├─ 智谱 → Groq 备用
   └─ Deezer / CATAAS / Wikimedia Commons / nailong-memes
   │
   ▼
...
| `@机器人 点歌 歌手名 + 歌名` | 所有人 | 搜索并发送 Deezer 30 秒试听音乐卡片 |
...
旧式 `/ai <问题>`、`/猫`、`/小猪`、`/点歌 <关键词>` 命令仍然兼容，但推荐直接 @ 机器人使用。
...
- 至少一个可用的 LLM API Key。
- 可选：Docker Desktop、Redis。Deezer 点歌、猫 GIF、真实猪图和奶龙图库均无需 API Key。
...
它会注册并立即运行 `QQChatRobot API` 计划任务，登录 Windows 后自动启动并守护 8000 服务。
...
- 普通群聊风格学习只记录匿名统计值与固定安全口头语计数。夺舍成功后会在后台读取 OneBot 最近历史，只筛选目标成员的纯文本来生成表达风格摘要；原始消息不落库，仅保存摘要，且不学习身份事实、隐私或辱骂。
...
- 随机和指向夺舍确认会立即回复，目标成员的历史语气学习在后台进行；样本不足或 OneBot 历史接口不可用时自动跳过，不阻塞聊天。
...
Deezer 公共搜索提供的是约 30 秒试听，不需要 Key；完整版权歌曲必须由持牌平台按地区和账号权限播放。机器人会发送可点击播放的试听音乐卡片和平台落地页，不使用盗链或音频解析接口。详见 [音乐点歌](docs/MUSIC.md)。
