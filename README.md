...
- 当前角色“小丛雨”的语音接口支持中文、日语和英语的语言分流，但语音默认使用中文。只有用户明确要求“请用英语回答”或“请用日语回答”时才切换对应语音前端；回复里夹带 AI、版本号、链接或外语短词不会切换语言。基础 v2 配置在先前测试中通过中文和英语 ASR 门槛；日语长句有同音正字法差异，历史严格门槛未通过，详见[语音验收说明](docs/MURASAME_MEDIA.md)。新微调权重仍须单独验收后才会启用。
...
| `可用角色` / `选择角色 小丛雨` | 所有人 | 查看或选择语音角色；小丛雨支持中 / 日 / 英 |
