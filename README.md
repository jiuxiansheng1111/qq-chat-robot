...
- 随机夺舍会从当天发言达到 15 条的群友中抽取“本群今日乐子”，可反复重抽；指向夺舍可以不限次数地切换到被 @ 的群成员。
...
| `@机器人 随机夺舍` | 所有人 | 从当天发言达到 15 条的成员中重新抽取“本群今日乐子” |
...
- 至少一个可用的 LLM API Key。
- 可选：Docker Desktop、Redis。网易云点歌、猫 GIF、真实猪图和奶龙图库均无需额外 API Key。
...
- 夺舍时暂停“小丛雨”的吾辈、汝、古风句尾等默认口癖，以目标成员历史风格为主；支持在当前状态中声明目标成员别名。
- 夺舍期间会持久化最近 20 轮被 @ 对话作为群内共享角色上下文，并记录每句话的说话者群名片，方便正确推理人物和辈分；切换目标或退出时自动清除。
- 每次随机或指向夺舍都会重新读取目标成员近期群历史，只筛选该成员纯文本并刷新语气摘要；普通对话最多等待 3 秒让学习完成，样本不足或 OneBot 历史接口不可用时自动跳过。
